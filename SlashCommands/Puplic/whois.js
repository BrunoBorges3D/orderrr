...
module.exports = {
    ...
    execute: async function(interaction, client) {
		...
		if (!member) return interaction.reply({ embeds: [new EmbedBuilder().setColor(client.config.embedError).setDescription(`❌ I couldn't find the user ${member}`)], ephemeral: true });
		...
		if (extra.length) {
			embed.fields.push({ name: 'KaDev Extra', value: extra.join(', '), inline: false });
		}

		if (team.length) {
			embed.fields.push({ name: 'KaDev Team', value: `${team.join(', ')}`, inline: false });
		}
